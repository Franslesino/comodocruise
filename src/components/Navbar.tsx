...
export default function Navbar() {
    ...
    const isHeroPage = pathWithoutLocale === "/" || pathWithoutLocale === "" || pathWithoutLocale === "/results";

    // isTransparent = true when Hero is visible (applies to pages with Hero)
    const [isTransparent, setIsTransparent] = useState(isHeroPage);
    const [navbarHeight, setNavbarHeight] = useState(0);
    const headerRef = useRef<HTMLElement>(null);
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    // isHidden state removed as user wants the navbar to stay visible at all times
    // Mobile Hide-on-scroll logic removed
    ...
    const isSolid = !isHeroPage || !isTransparent;
    ...
    return (
        <>
            ...
            {!isHeroPage && <div style={{ height: navbarHeight }} />}

            <header
                ref={headerRef}
                className={`transition-all duration-300 ease-out z-50 w-full ${positionClass} ${bgClass} translate-y-0`}
            >
                <div className={`flex items-center justify-between px-5 md:px-10 ${paddingClass}`}>
                    ...
                    <div className="flex items-center gap-2 md:gap-4">
                        ...
                        >
                            <Image
                                ...
                                width={40}
                                height={40}
                                className={`w-8 h-8 md:w-10 md:h-10 object-contain transition-all duration-300 ${isTransparent ? 'brightness-0 invert' : ''
                                    }`}
                            />
                        </LocaleLink>
                    ...
                    {/* Center: filter slot for cruises page + BookingBar when scrolled */}
                    <div className="flex-1 flex justify-center mx-4">
                        <div id="navbar-filter-slot" />
                        <div id="navbar-booking-container" className="flex justify-center w-full">
